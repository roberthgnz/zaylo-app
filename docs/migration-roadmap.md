...
- **Store catalog / public storefront / product detail duality confirmed but not acted on.** Web's `dashboard/store/[username]/**` is literally re-exported at `app/[locale]/[username]/**` — the seller's own catalog view and the public-facing storefront share components. This pass only built the seller-facing "manage my own catalog" screen (`dashboard-store/index.tsx`); the public storefront (any buyer browsing `zaylo.bio/{username}`) and product detail page remain their own deferred phase, unblocked by this one but not started.
